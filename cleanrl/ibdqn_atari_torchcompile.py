# Implicit-Baseline DQN (IB-DQN), torch.compile / CUDA-graph twin of
# cleanrl/ibdqn_atari.py. Mean-expansion layer from "Accelerating Q-learning
# through Efficient Value-Sharing across Actions", Nagarajan, Daley, White &
# Machado, ICML 2026 (arXiv:2606.29806): q = (I + (k/n) J) z, no learnable
# parameters, k = 0 is the identity (plain DQN) to within float round-off,
# default k = n.
#
...
class QNetwork(nn.Module):
    def __init__(self, envs, device=None, mean_scaling_coefficient=-1.0):
        super().__init__()
        if envs.single_observation_space.shape != (4, 84, 84):
            raise ValueError(
                "ibdqn_atari_torchcompile.py expects four stacked 84x84 Atari frames, "
...
if __name__ == "__main__":
    ...
    q_network = QNetwork(envs, device=device, mean_scaling_coefficient=args.mean_scaling_coefficient)
    q_network_detach = QNetwork(envs, device=device, mean_scaling_coefficient=args.mean_scaling_coefficient)
    ...
    target_network = QNetwork(envs, device=device, mean_scaling_coefficient=args.mean_scaling_coefficient)
