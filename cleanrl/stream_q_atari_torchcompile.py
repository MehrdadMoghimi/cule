# torch.compile twin of stream_q_atari.py.
#
...
"""Stream Q(lambda) on Atari with optional torch.compile and CUDA graphs.

Every tensor region here is fixed-shape and free of data-dependent control flow
-- the trace reset is an unconditional mask multiply and the ObGD step size is
computed elementwise rather than through `.item()` -- so the policy and the
whole streaming update (per-stream `vmap(grad(...))` included) can be compiled
and captured.  Environment stepping and the running normaliser stay eager.
...
if __name__ == "__main__":
    ...
    parameter_names = [name for name, _ in q_network.named_parameters()]

    def policy(observations: torch.Tensor, epsilon: torch.Tensor):
        ...
        return actions, actions != greedy_actions
    ...
    def stream_update(
        ...
        with torch.no_grad():
            q_sa = q_network(observations).gather(1, actions.view(-1, 1)).squeeze(1)
            next_max_q = q_network(next_observations).max(dim=-1).values
        ...
        grads = per_stream_grad(params, observations, F.one_hot(actions, n_actions).float())
        step_size = optimizer.step([grads[name] for name in parameter_names], delta, reset)
        return delta, q_sa, step_size
    ...
    if args.cudagraphs:
        ...
        # The eligibility traces and parameters are module/optimizer state that
        # the capture mutates in place, so replays carry the stream forward.
    ...
    for vector_step in range(num_vector_steps):
        ...
        with torch.no_grad():
            actions, non_greedy = policy(obs, epsilon_tensor)
        ...
        if args.compile:
            torch.compiler.cudagraph_mark_step_begin()
        last_delta, q_sa, last_step_size = stream_update(
            obs,
            next_obs,
            actions,
            rewards,
