...
def shrink_perturb_and_reset(q_network, target_network, optimizer, args, envs, device):
...
if __name__ == "__main__":
    ...
    optimizer = optim.AdamW(
        q_network.parameters(), lr=args.learning_rate, eps=args.adam_eps, weight_decay=args.weight_decay
    )
    ...
    for vector_step in range(num_vector_steps):
        ...
        with torch.no_grad():
            latent = target_network.encode(to_tensor(obs, device).float())
        ...
        if (
            ...
            for _ in range(num_updates):
                ...
                with torch.no_grad():
                    ...
                    gamma_n = current_gamma**current_n
                ...
                log_pred = log_dist[batch_indices, data.actions.flatten()]
                ...
                loss = (rl_loss_per_sample * flat_weights).mean() + args.spr_weight * (
                    spr_loss_per_sample * flat_weights
                ).mean()

                rb.update_priorities(indices, rl_loss_per_sample.detach().cpu().numpy())
                ...
                if learner_updates >= next_reset and learner_updates <= args.no_resets_after:
                    shrink_perturb_and_reset(q_network, target_network, optimizer, args, envs, device)
                    cycle_grad_steps = 0
                    next_reset += args.reset_every

            if not args.benchmark and global_step >= next_log_step and num_updates:
                q_values = (log_pred.exp() * q_network.support).sum(dim=1)
                sps = int(global_step / (time.time() - start_time))
                writer.add_scalar("losses/rl_loss", rl_loss_per_sample.mean().item(), global_step)
                writer.add_scalar("losses/spr_loss", spr_loss_per_sample.mean().item(), global_step)
