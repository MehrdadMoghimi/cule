...
# https://arxiv.org/abs/2004.13649) with optional torch.compile, CUDA graphs,
...
"""DrQ(eps) Atari-100K with optional torch.compile and GPU TorchRL replay.

Ported from the official Dopamine implementation
(https://github.com/google/dopamine, dopamine/labs/atari_100k,
configs/DrQ_eps.gin).  Defaults reproduce the official single-environment
configuration; see drq_atari.py for the eager twin.  Environment stepping and
n-step accumulation stay outside the compiled regions.  Replay is uniform;
n-step windows that cross an episode boundary are stored with a zero loss
weight instead of being filtered, keeping sample shapes static for capture.
DrQ augmentation runs inside the compiled update.  Supports the cule,
envpool, and gymnasium backends.
"""
...
from cleanrl_utils.torchrl_replay import NStepTransitionAccumulator
...
if __name__ == "__main__":
    ...
    def update(data: TensorDict) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            td_target = rewards + gamma_n * next_q * (1 - dones)

        q_value = q_network(observations).gather(1, actions.unsqueeze(-1)).squeeze(-1)
        per_sample = F.huber_loss(q_value, td_target, reduction="none")
        # n-step windows crossing an episode boundary carry zero weight
        loss = (per_sample * valid).sum() / valid.sum().clamp(min=1.0)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        return loss.detach(), q_value.detach().mean()
    ...
    if args.cudagraphs:
        ...
        # The per-update target-network sync mutates module tensors in place,
        # so graph replays observe it; sampling and stepping stay outside.
    ...
    for vector_step in range(num_vector_steps):
        ...
        if n_step_transition is not None:
            ...
            n_step_data["valid"] = valid
            rb.extend(n_step_data)
    ...
    if args.benchmark:
        ...
        result = {
            ...
            "replay_backend": "torchrl_lazy_tensor_storage_uniform_nstep",
