# FQF: Fully Parameterized Quantile Function for Distributional Reinforcement
# Learning (Yang et al., 2019, https://arxiv.org/abs/1911.02140).
# The fraction-proposal gradient and surrogate backward follow the official
# authors' implementation (https://github.com/microsoft/FQF); widely used
# third-party ports differ here and were deliberately not followed.
...
if __name__ == "__main__":
    ...
    q_network = QNetwork(envs, n_quantiles=args.n_quantiles, n_cos=args.n_cos).to(device)
    ...
    optimizer = optim.Adam(quantile_parameters, lr=args.learning_rate, eps=0.01 / args.batch_size)
    ...
    target_network = QNetwork(envs, n_quantiles=args.n_quantiles, n_cos=args.n_cos).to(device)
    ...
    for vector_step in range(num_vector_steps):
        ...
        if global_step > args.learning_starts and len(rb) >= args.batch_size:
            ...
            for _ in range(num_updates):
                ...
                batch_actions = data.actions.flatten()

                # The fraction proposals come from the online network on the
                # current states; detached features keep the conv trunk out of
                # the fraction-loss graph.
                features = q_network.features(data.observations)
                taus, tau_hats, entropy = q_network.fraction_proposal(features.detach())

                with torch.no_grad():
                    # As in the official FQF, the target network proposes its
                    # own fractions for the next states.
                ...
                old_quantiles = z[batch_indices, :, batch_actions]
                ...
                rho = (
                    (tau_hats.detach().unsqueeze(-1) - (u.detach() < 0).float()).abs()
                    * huber
                    / args.kappa
                )
                ...
                with torch.no_grad():
                    z_taus = q_network.quantile_values(features, taus[:, 1:-1])[
                        batch_indices, :, batch_actions
                    ]
                ...
                # optimize the model; the two losses touch disjoint parameters
                # (tau_hats are detached in the quantile loss; features and
                # quantile values are detached in the fraction loss), so one
                # backward over the sum equals two backwards
                optimizer.zero_grad()
            ...
            if not args.benchmark and global_step >= next_log_step and num_updates:
                ...
                writer.add_scalar("losses/loss", quantile_loss.item(), global_step)
                writer.add_scalar("losses/fraction_loss", fraction_loss.item(), global_step)
                writer.add_scalar("losses/fraction_entropy", entropy.mean().item(), global_step)
