...
if __name__ == "__main__":
    ...
    encoder_optimizer = optim.AdamW(
        encoder.parameters(), lr=args.encoder_learning_rate, weight_decay=args.weight_decay
    )
    value_optimizer = optim.AdamW(
        value.parameters(), lr=args.value_learning_rate, weight_decay=args.weight_decay
    )
    policy_optimizer = optim.AdamW(
        policy.parameters(), lr=args.policy_learning_rate, weight_decay=args.weight_decay
    )
    ...
    def update_encoder(data: MRQSamples, done_weight: float):
        with torch.no_grad():
            flat_next = data.next_observations.flatten(0, 1).float()
            if args.data_augmentation:
                flat_next = random_shift_augmentation(flat_next)
            target_zs = encoder_target.zs(flat_next).view(data.observations.shape[0], -1, args.zs_dim)
    ...
    def update_rl(data: MRQSamples, reward_scale: float, target_reward_scale: float):
        observations = data.observations.float()
    ...
    update_budget = 0.0
    learner_updates = 0
    reward_scale, target_reward_scale = 1.0, 0.0
    ...
    for vector_step in range(num_vector_steps):
        ...
        if global_step < args.learning_starts:
            actions = torch.randint(n_actions, (args.num_envs,), device=device)
        else:
            with torch.no_grad():
                relaxed = policy.act(encoder.zs(to_tensor(obs, device).float()))
                relaxed = relaxed + torch.randn_like(relaxed) * exploration_noise
                actions = relaxed.argmax(1)
        ...
        if (
            ...
            for _ in range(num_updates):
                ...
                if (learner_updates - 1) % args.target_network_frequency == 0:
                    ...
                    done_weight = args.done_weight if rb.env_terminates else 0.0
                    for _ in range(args.target_network_frequency):
                        last_encoder_loss = update_encoder(
                            rb.sample_subtrajectory(args.batch_size, args.enc_horizon, True),
                            done_weight,
                        )
                ...
                last_value_loss, last_policy_loss, last_q_value, priority = update_rl(
                    data, reward_scale, target_reward_scale
                )
            ...
            if not args.benchmark and global_step >= next_log_step and num_updates:
                ...
                writer.add_scalar("charts/reward_scale", reward_scale, global_step)
