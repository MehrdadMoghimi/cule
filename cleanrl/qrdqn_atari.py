...
class QNetwork(nn.Module):
    def __init__(self, env, n_quantiles=200):
        ...
        self.network = nn.Sequential(
            ...
            nn.Linear(3136, 512),
            nn.ReLU(),
            nn.Linear(512, self.n * n_quantiles),
        )

    def get_action(self, x, action=None):
        # quantile values theta_i(x, a) for each action
        quantiles = self.network(x / 255.0).view(len(x), self.n, self.n_quantiles)
    ...
    for vector_step in range(num_vector_steps):
        ...
        if global_step > args.learning_starts and len(rb) >= args.batch_size:
            ...
            for _ in range(num_updates):
                ...
                rho = (
                    (q_network.tau_hats.view(1, -1, 1) - (u.detach() < 0).float()).abs()
                    * huber
                    / args.kappa
                )
