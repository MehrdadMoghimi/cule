# torch.compile twin of btr_atari.py.
#
# BTR: Beyond The Rainbow (Clark et al., ICML 2025,
# https://arxiv.org/abs/2411.03820), ported from the official implementation
# (https://github.com/VIPTankz/BTR).  BTR's learning rule *is* Munchausen-IQN,
# so this file is miqn_atari_torchcompile.py with the encoder swapped for the
# spectrally-normalised Impala trunk and the uniform replay swapped for the
# n-step prioritized replay that rainbow_atari_torchcompile.py already uses.
# See btr_atari.py for the full list of inherited vs. changed components and
# for the documented deviations from the paper text.
#
# The trainer skeleton derives from CleanRL
...
"""BTR Atari with optional torch.compile, CUDA graphs, and GPU TorchRL replay.
...
if __name__ == "__main__":
    ...
    optimizer = optim.Adam(
        ...
        eps=args.adam_eps_ratio / args.batch_size,
        capturable=args.cudagraphs and not args.compile,
    )
    # BTR replaces M-IQN's uniform replay with n-step PER.  The sampler supplies
    # the priority tree on the GPU while TorchRL owns storage and cyclic writes,
    # exactly as in rainbow_atari_torchcompile.py.
    ...
    def policy(observations: torch.Tensor, epsilon: torch.Tensor) -> torch.Tensor:
        ...
        if args.interact == "stochastic":
            ...
            advantages = q_network.quantile_values(features, policy_taus, advantages_only=True).mean(1)
    ...
    def update(data: TensorDict) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            # One target draw serves both the bootstrap quantiles and the target
            # policy: BTR takes `q_t_n = Q_targets_next.mean(1)`.
            ...
            # Munchausen bonus: alpha * clip(tau * ln pi(a|s), l0, 0).  BTR reads
            # the log-policy off the ONLINE network at s_t, unlike M-IQN.
            current_q = q_network.quantile_values(
                q_network.features(observations),
                torch.rand(observations.shape[0], args.n_policy_taus, device=observations.device),
            ).mean(1)
            target_quantiles = munchausen_target(
                current_q, next_q, next_z, actions.reshape(-1, 1), rewards, dones,
        ...
        loss = (quantile_huber_loss(u, taus, args.kappa) * weights).mean()
        # Priority is the summed-then-averaged absolute TD error.
        priority = u.abs().mean(2).sum(1)
        optimizer.zero_grad()
        loss.backward()
        nn.utils.clip_grad_norm_(q_network.parameters(), args.max_grad_norm)
        optimizer.step()
        q_value = old_quantiles.mean()
        return loss.detach(), priority.detach(), q_value.detach()
    ...
    for vector_step in range(num_vector_steps):
        ...
        # BTR anneals epsilon over a fixed transition budget and switches
        # epsilon-greedy off entirely at the halfway point, leaving the noisy
        # layers as the only source of exploration.
        epsilon_tensor.fill_(
            0.0
            if global_step >= epsilon_disable_step
            else linear_schedule(args.start_e, args.end_e, args.epsilon_decay_steps, global_step)
        )
        # Resample the factorised noise once per acting step, outside any
        # captured region: the epsilon buffers are module state, so graph
        # replays observe the new draw.
    ...
    if args.save_model and not args.benchmark:
        ...
        if args.upload_model:
            ...
            push_to_hub(args, episodic_returns, repo_id, "BTR", f"runs/{run_name}", f"videos/{run_name}-eval")
