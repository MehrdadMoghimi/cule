...
"""Stream AC(lambda) on Atari: replay-free, target-network-free, batch-free.
...
if __name__ == "__main__":
    ...
    for vector_step in range(num_vector_steps):
        ...
        with torch.no_grad():
            value = value_network(obs).squeeze(-1)
            next_value = value_network(next_obs).squeeze(-1)
        ...
        policy_grads = per_stream_policy_grad(
            policy_params, obs, action_onehot, torch.sign(delta), args.entropy_coeff
        )
        value_params = {name: p.detach() for name, p in value_network.named_parameters()}
        value_grads = per_stream_value_grad(value_params, obs)

        # The trace is cut on termination only: stream AC is on-policy, so there
        # is no exploratory action to cut it for.
        last_step_size = policy_optimizer.step(
            [policy_grads[name] for name in policy_parameter_names], delta, transition_dones
        )
        value_optimizer.step(
            [value_grads[name] for name in value_parameter_names], delta, transition_dones
        )
        last_delta = delta
