# QR-DQN (Dabney et al., 2017, https://arxiv.org/abs/1710.10044) with optional
...
# (google/dopamine, dopamine/jax/agents/quantile/quantile_agent.py); see
# qrdqn_atari.py for the eager twin.  The trainer skeleton derives from CleanRL
...
"""QR-DQN Atari with optional torch.compile and GPU-resident TorchRL replay.
...
class QNetwork(nn.Module):
    def __init__(self, env, n_quantiles=200):
        super().__init__()
        self.n_quantiles = int(n_quantiles)
        # Quantile midpoints tau_hat_i = (2i + 1) / (2N).
        self.register_buffer(
            "tau_hats", (2 * torch.arange(n_quantiles, dtype=torch.float32) + 1) / (2.0 * n_quantiles)
        )
        ...
        self.network = nn.Sequential(
            ...
            nn.Linear(3136, 512),
            nn.ReLU(),
            nn.Linear(512, self.n_actions * n_quantiles),
        )
    ...
    def get_action(self, x: torch.Tensor, action: torch.Tensor | None = None):
        ...
        quantiles = self.network(x / 255.0).reshape(-1, self.n_actions, self.n_quantiles)
...
if __name__ == "__main__":
    ...
    q_network = QNetwork(envs, args.n_quantiles).to(device)
    target_network = QNetwork(envs, args.n_quantiles).to(device)
    ...
    def update(data: TensorDict) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            _, next_quantiles = target_network.get_action(next_observations)
            target_quantiles = rewards + args.gamma * next_quantiles * (1 - dones)

        _, old_quantiles = q_network.get_action(observations, actions.flatten())
        ...
        rho = (q_network.tau_hats.view(1, -1, 1) - (u.detach() < 0).float()).abs() * huber / args.kappa
    ...
    if args.save_model and not args.benchmark:
        ...
        if args.upload_model:
            ...
            push_to_hub(args, episodic_returns, repo_id, "QRDQN", f"runs/{run_name}", f"videos/{run_name}-eval")
