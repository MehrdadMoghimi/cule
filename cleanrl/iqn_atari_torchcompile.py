# IQN (Dabney et al., 2018, https://arxiv.org/abs/1806.06923) with optional
...
# dopamine/jax/agents/implicit_quantile/implicit_quantile_agent.py); see
# iqn_atari.py for the eager twin.  The trainer skeleton derives from CleanRL
    ...
    def update(data: TensorDict) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            ...
            action_taus = torch.rand(
                next_features.shape[0], args.n_policy_taus, device=next_features.device
            )
            ...
            next_taus = torch.rand(
                next_features.shape[0], args.n_target_taus, device=next_features.device
            )
