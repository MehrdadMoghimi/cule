...
#
# This is the torch.compile / CUDA-graph twin of disco_atari.py, following
# LeanRL's structure (https://github.com/meta-pytorch/LeanRL, MIT). Two
# fixed-shape regions are compiled: the behavior policy, and the whole learner
# step (value targets, the frozen meta-network, the loss, the optimizer step and
# the Polyak target update). Both recurrences inside the meta-network are Python
# loops over a fixed horizon, so they unroll into the graph -- which makes
# `--num-steps` the dominant term in compile time.
#
...
class Args:
    ...
    compile: bool = False
    """whether to compile the policy and the learner step"""
    cudagraphs: bool = False
    """whether to wrap the policy and the learner step in CudaGraphModule"""
    ...
    def step(self, closure=None):
        for group in self.param_groups:
            ...
            for parameter in group["params"]:
                ...
                if not state:
                    # The step counter lives on the device so that bias
                    # correction stays a tensor op: a Python int would be baked
                    # into a captured graph as a constant.
                ...
                mu_hat = mu / (1.0 - torch.pow(beta1, state["step"]))
                nu_hat = nu / (1.0 - torch.pow(beta2, state["step"]))
...
if __name__ == "__main__":
    ...
    def policy_fn(current_obs: torch.Tensor):
        logits = agent(current_obs)["logits"]
        return logits, torch.multinomial(F.softmax(logits, dim=-1), 1).squeeze(-1)

    def update(batch: TensorDict):
        batch_obs = batch["observations"]
        batch_actions = batch["actions"]
        batch_logits = batch["behaviour_logits"]
        batch_rewards = batch["rewards"]
        batch_terminal = batch["is_terminal"]
        ...
        # set_to_none=False keeps the gradient buffers at fixed addresses for a
        # captured graph to replay into.
        ...
        return loss.detach(), normalized_q_td.mean(), *log.values()

    log_names = ("loss", "normalized_q_td", "pi_loss", "y_loss", "z_loss", "aux_loss", "value_loss")
    ...
    if args.cudagraphs:
        ...
        # The meta-network's lifetime LSTM state, the EMA normalizers and the
        # Polyak target update all mutate tensors in place, so graph replays
        # observe them.
    ...
    def act(current_obs):
        ...
        with torch.no_grad():
            return policy_fn(to_tensor(current_obs, device))
        ...
        if len(replay) >= args.batch_size:
            for _ in range(args.learner_steps_per_rollout):
                ...
                batch = TensorDict(
                    dict(
                        zip(
                            ("observations", "actions", "behaviour_logits", "rewards", "is_terminal"),
                            (torch.as_tensor(array, device=device) for array in sampled),
                        )
                    ),
                    # The rollout entries do not share a prefix: observations,
                    # actions and logits span T+1 steps, rewards and terminals T.
                    batch_size=[],
                    device=device,
                )
                if args.compile:
                ...
                learner_steps += 1
                last_log = dict(zip(log_names, outputs))
