# FQF (Yang et al., 2019, https://arxiv.org/abs/1911.02140) with optional
# torch.compile, CUDA graphs, and GPU-resident TorchRL replay.
# The fraction-proposal gradient and surrogate backward follow the official
# authors' implementation (https://github.com/microsoft/FQF); see fqf_atari.py
# for the eager twin.  The trainer skeleton derives from CleanRL
# (https://github.com/vwxyzjn/cleanrl, MIT) and the compile / CUDA-graph
# structure from LeanRL (https://github.com/meta-pytorch/LeanRL, MIT); both
# licenses are reproduced in cleanrl/LICENSE.md.
"""FQF Atari with optional torch.compile and GPU-resident TorchRL replay.
...
The learner update runs both optimizers (Adam for the quantile network,
RMSprop for the fraction-proposal network) inside the compiled/captured
region.  Supports the cule, envpool, and gymnasium backends.
"""
...
class Args:
    ...
    """the learning rate of the quantile-network optimizer"""
    fraction_learning_rate: float = 2.5e-9
    """the learning rate of the fraction-proposal RMSprop optimizer"""
    fraction_entropy_coef: float = 0.0
    """entropy regularization coefficient for the fraction proposals"""
    num_envs: int = 256
    """the number of parallel game environments"""
    n_quantiles: int = 32
    """the number of proposed quantile fractions"""
    n_cos: int = 64
...
class FractionProposalNetwork(nn.Module):
    def __init__(self, feature_dim, n_quantiles):
        super().__init__()
        self.linear = nn.Linear(feature_dim, n_quantiles)
        nn.init.xavier_uniform_(self.linear.weight, gain=0.01)
        nn.init.zeros_(self.linear.bias)

    def forward(self, features):
        log_probs = torch.log_softmax(self.linear(features), dim=-1)
        probs = log_probs.exp()
        taus = F.pad(torch.cumsum(probs, dim=-1), (1, 0))
...
class QNetwork(nn.Module):
    def __init__(self, env, n_quantiles=32, n_cos=64):
        super().__init__()
        ...
        self.fraction_proposal = FractionProposalNetwork(self.feature_dim, self.n_quantiles)
    ...
    def get_action(self, x: torch.Tensor, action: torch.Tensor | None = None):
        ...
        taus, tau_hats, _ = self.fraction_proposal(features.detach())
        quantiles = self.quantile_values(features, tau_hats)
        q_values = ((taus[:, 1:] - taus[:, :-1]).unsqueeze(-1) * quantiles).sum(1)
        if action is None:
...
if __name__ == "__main__":
    ...
    q_network = QNetwork(envs, args.n_quantiles, args.n_cos).to(device)
    target_network = QNetwork(envs, args.n_quantiles, args.n_cos).to(device)
    target_network.load_state_dict(q_network.state_dict())
    capturable = args.cudagraphs and not args.compile
    # The fraction-proposal network trains with its own RMSprop optimizer, as in
    # the paper; the Adam optimizer covers only the quantile-value parameters.
    quantile_parameters = [
        parameter
        for name, parameter in q_network.named_parameters()
        if not name.startswith("fraction_proposal.")
    ]
    ...
    fraction_optimizer = optim.RMSprop(
        q_network.fraction_proposal.parameters(),
        lr=args.fraction_learning_rate,
        alpha=0.95,
        eps=1e-5,
        centered=True,
        capturable=capturable,
    ...
    def update(data: TensorDict) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # The fraction proposals come from the online network on the current
        # states; detached features keep the conv trunk out of the
        # fraction-loss graph.
        features = q_network.features(observations)
        taus, tau_hats, entropy = q_network.fraction_proposal(features.detach())

        with torch.no_grad():
            # As in the official FQF, the target network proposes its own
            # fractions for the next states.
            next_features = target_network.features(next_observations)
            next_taus, next_tau_hats, _ = target_network.fraction_proposal(next_features)
            next_z = target_network.quantile_values(next_features, next_tau_hats)
            next_q = ((next_taus[:, 1:] - next_taus[:, :-1]).unsqueeze(-1) * next_z).sum(1)
            next_actions = torch.argmax(next_q, dim=1)
            next_quantiles = next_z[batch_indices, :, next_actions]
            target_quantiles = rewards + args.gamma * next_quantiles * (1 - dones)

        z = q_network.quantile_values(features, tau_hats.detach())
        old_quantiles = z[batch_indices, :, actions]
        ...
        rho = (tau_hats.detach().unsqueeze(-1) - (u.detach() < 0).float()).abs() * huber / args.kappa
        quantile_loss = rho.mean(2).sum(1).mean()

        # W1 fraction loss: dW1/dtau_i = 2 z(tau_i) - z(tau_hat_{i-1}) - z(tau_hat_i)
        with torch.no_grad():
            z_taus = q_network.quantile_values(features, taus[:, 1:-1])[batch_indices, :, actions]
            tau_gradients = 2 * z_taus - old_quantiles[:, :-1] - old_quantiles[:, 1:]
        fraction_loss = (tau_gradients * taus[:, 1:-1]).sum(1).mean()
        if args.fraction_entropy_coef != 0.0:
            fraction_loss = fraction_loss - args.fraction_entropy_coef * entropy.mean()

        optimizer.zero_grad()
        fraction_optimizer.zero_grad()
        # The two losses touch disjoint parameters (tau_hats are detached in
        # the quantile loss; features and quantile values are detached in the
        # fraction loss), so one backward over the sum equals two backwards
        # and keeps the region compilable.
        (quantile_loss + fraction_loss).backward()
        optimizer.step()
        fraction_optimizer.step()
        q_value = old_quantiles.mean()
        return quantile_loss.detach(), fraction_loss.detach(), q_value.detach()
    ...
    for vector_step in range(num_vector_steps):
        ...
        if global_step > args.learning_starts and len(rb) >= args.batch_size:
            ...
            for _ in range(num_updates):
                if args.compile:
                    torch.compiler.cudagraph_mark_step_begin()
                last_loss, last_fraction_loss, last_q_value = update(rb.sample(args.batch_size))
                learner_updates += 1
            ...
            if not args.benchmark and global_step >= next_log_step and num_updates:
                ...
                writer.add_scalar("losses/fraction_loss", last_fraction_loss.item(), global_step)
    ...
    if args.save_model and not args.benchmark:
        ...
        episodic_returns = evaluate(
            ...
            Model=QNetwork,
            model_kwargs_keys=("n_quantiles", "n_cos"),
            device=device,
            epsilon=args.end_e,
        )
        ...
        if args.upload_model:
            ...
            push_to_hub(args, episodic_returns, repo_id, "FQF", f"runs/{run_name}", f"videos/{run_name}-eval")
