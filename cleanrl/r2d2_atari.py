...
# CleanRL (https://github.com/vwxyzjn/cleanrl, MIT; license in
# cleanrl/LICENSE.md).
"""R2D2 on Atari: recurrent replay with stored state and burn-in.
...
if __name__ == "__main__":
    ...
    for vector_step in range(num_vector_steps):
        ...
        with torch.no_grad():
            q_values, state = q_network.step(obs, previous_actions, previous_rewards, state)
            greedy_actions = torch.argmax(q_values, dim=1)
        random_actions = torch.randint(n_actions, (args.num_envs,), device=device)
        explore = torch.rand(args.num_envs, device=device) < epsilons
        ...
        if (
            ...
            for _ in range(num_updates):
                ...
                start_state = (data["hidden"], data["cell"])
                ...
                with torch.no_grad():
                    q_target_values, _ = target_network.unroll(
                        ...
                        data["previous_dones"],
                        start_state,
                        grad_from=data["observations"].shape[1],
                    )
                ...
                predictions = (
                    q_online[:, horizon].gather(2, data["actions"][:, horizon].unsqueeze(-1)).squeeze(-1)
                )
                td_errors = targets - predictions
                loss = (0.5 * td_errors.pow(2).mean(dim=1) * data["weights"]).mean()
                ...
                rb.update_priorities(data["indices"], priorities.cpu().numpy())
                ...
                last_q_value = predictions.detach().mean()
