...
# The IQN base and trainer structure follow iqn_atari.py / c51_atari.py, which
# are adapted from CleanRL (https://github.com/vwxyzjn/cleanrl, MIT; license in
# cleanrl/LICENSE.md).  Supports gymnasium, cule, and envpool.
import json
import math
import os
import random
        ...
        if global_step > args.learning_starts and len(rb) >= args.batch_size:
            ...
            for _ in range(num_updates):
                ...
                with torch.no_grad():
                    ...
                    current_features = target_network.features(data.observations)
                    ...
                    target_quantiles = (
                        data.rewards + munchausen_bonus + args.gamma * soft_values * (1 - data.dones)
                    )
